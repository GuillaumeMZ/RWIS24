...
export default function AdminBase() {
  ...
  const { barbecues, addBarbecue, deleteBarbecue, selectBarbecue } = useBarbecueContext(); // Access context
  ...
  const handleCreateBarbecue = () => {
    ...
    addBarbecue(newBarbecue); // Add the new barbecue to context
    setNewBarbecue({ name: "", date: "", description: "", accessCode: "" });
    setIsCreating(false);
  };

  const handleNavigateToBarbecue = (barbecue) => {
    selectBarbecue(barbecue.id); // Mark this barbecue as selected
    router.push("./barbecue/ManagingBarbecue"); // Navigate to ManagingBarbecue screen
  };
